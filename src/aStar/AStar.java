...
public class AStar {
        ...
        public Path calcCoolestPath(int startX, int startY, int goalX, int goalY) {
            ...
            // On crée la liste des point à ouvrir
            ...
            while(rechercheList.size() != 0)
            {
                    ...
                    if(current.getX() == map.getGoalLocationX() && current.getY() == map.getGoalLocationY())
                    {
                    	// On renvoi simplement l'itinéraire (déjà construit) dans l'ordre.
                    	return reconstructPath(current);
                    }

                    // Au premier tour on supprimer le départ pour l'ajouter à closedList qui contient l'tineraire
        ...
        public void printItineraire()
        {
            ...
            for (int i=0;i<shortestPath.getLength();i++)
            {
            	...
            	{
            		System.out.println("Arrêt n°" + numStat + " : " + map.getNode(x, y).nomStat + " (" +  map.getNode(x, y).nomLign + ")");
            	}
            	else
            	{
